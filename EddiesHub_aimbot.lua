...
if type(_G.__eddiesHubCleanup) == "function" then
	pcall(_G.__eddiesHubCleanup);
end;
_G.__eddiesHubCleanup = nil;
...
_G.__eddiesHubLockOpt = type(_G.__eddiesHubLockOpt) == "table" and _G.__eddiesHubLockOpt or {};
VLO = _G.__eddiesHubLockOpt;
VLO.aimOrigin = {};
VLO.input = {};
...
function npc.dispose(force)
	local cache = _G.__eddiesHubNpcCache;
	...
	if _G.__eddiesHubNpcCache == cache then
		_G.__eddiesHubNpcCache = nil;
	end;
end;
...
function npc.make()
	local old = _G.__eddiesHubNpcCache;
...
local cfgFile = "EddiesHub-Aimbot/config.json";
...
local function getCfgPath(name)
	return "EddiesHub-Aimbot/configs/" .. cleanCfgName(name) .. ".json";
end;
local function saveMeta()
	if not writefile or (not HS) then
		return;
	end;
	if isfolder and (not isfolder("EddiesHub-Aimbot")) then
		if makefolder then
			pcall(makefolder, "EddiesHub-Aimbot");
		else
			return;
		end;
	end;
...
local function getConfigNames()
	...
	if listfiles and isfolder and isfolder("EddiesHub-Aimbot/configs") then
		for _, path in listfiles("EddiesHub-Aimbot/configs") do
...
local function cleanup()
	for _, g in uiRoot:GetChildren() do
		if g:IsA("ScreenGui") and (g.Name == "EddiesHub" or getUiTag(g) == "EddiesHub") then
			g:Destroy();
		end;
	end;
end;
...
VLO.aimOrigin.saveCfg = function(forceSave, nameOverride)
	...
	if isfolder and (not isfolder("EddiesHub-Aimbot")) then
		if makefolder then
			local s, e = pcall(makefolder, "EddiesHub-Aimbot");
	...
	if isfolder and (not isfolder("EddiesHub-Aimbot/configs")) then
		if makefolder then
			local s = pcall(makefolder, "EddiesHub-Aimbot/configs");
			if not s then
				return;
			end;
		else
...
local function resetAimCaches(resetTrackedTarget)
	...
	local npcCache = _G.__eddiesHubNpcCache;
...
unbindStrongInputs = function()
	pcall(function()
		CAS:UnbindAction("EddiesHubLock");
	end);
	pcall(function()
		CAS:UnbindAction("EddiesHubHotkeys");
	end);
	for _, fieldName in {
		"hotkeyRawCon",
		"hotkeyChangedCon",
...
rebindStrongInputs = function()
	...
	if lockInput then
		...
		local ok = pcall(function()
			CAS:BindActionAtPriority("EddiesHubLock", uiRefs.lockAct, false, Enum.ContextActionPriority.High.Value + 1000, lockInput);
		end);
		if not ok then
			pcall(function()
				CAS:BindAction("EddiesHubLock", uiRefs.lockAct, false, lockInput);
			end);
		end;
	end;
	...
	if #hotkeyInputs > 0 then
		...
		local ok = pcall(function()
			CAS:BindActionAtPriority("EddiesHubHotkeys", uiRefs.hotkeyAct, false, Enum.ContextActionPriority.High.Value + 1000, unpack(hotkeyInputs));
		end);
		if not ok then
			pcall(function()
				CAS:BindAction("EddiesHubHotkeys", uiRefs.hotkeyAct, false, unpack(hotkeyInputs));
			end);
		end;
	end;
...
VLO.aimOrigin.toast("Eddie's Hub loaded");
VLO.aimOrigin.saveCfg();
...
_G.__eddiesHubCleanup = function()
	npc.dispose(false);
	for _, c in conns do
		disconnectConn(c);
	end;
